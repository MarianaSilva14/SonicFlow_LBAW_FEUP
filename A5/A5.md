...
| R07                | Answer(idParent → Commentary, idChild → Commentary) |
| R08                | Flagged(idComment → Commentary, hidden NN) |
...
As all relations schemas are in the Boyce–Codd Normal Form (BCNF), the relational schema is also in the BCNF and therefore there is no need to be refined using normalisation. They are in the BCNF because in every case the left side of the functional dependency is a superkey.
